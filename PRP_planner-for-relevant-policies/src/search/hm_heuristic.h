...
protected:
    ...
    // h^m table
    map<IntPairVector, int> hm_table;
    bool was_updated;

    void init_hm_table(IntPairVector &t);
    void update_hm_table();
    int eval(IntPairVector &t);
    int update_hm_entry(IntPairVector &t, int val);
    void extend_tuple(IntPairVector &t, const Operator &op);

    // some helper methods
    int check_tuple_in_tuple(const IntPairVector &tup, const IntPairVector &big_tuple);
    void state_to_tuple(const State &state, IntPairVector &t) {
    ...
    void get_operator_pre(const Operator &op, IntPairVector &t) {
        for (int i = 0; i < op.get_prevail().size(); i++)
    ...
    void get_operator_eff(const Operator &op, IntPairVector &t) {
        for (int i = 0; i < op.get_pre_post().size(); i++)
    ...
    void generate_all_tuples(int var, int sz, IntPairVector &base);

    void generate_all_partial_tuple(IntPairVector &base_tuple, vector<IntPairVector> &res) {
        IntPairVector t;
        generate_all_partial_tuple(base_tuple, t, 0, m, res);
    }
    void generate_all_partial_tuple(IntPairVector &base_tuple, IntPairVector &t, int index, int sz, vector<IntPairVector> &res);

    void dump_table() const {
        map<IntPairVector, int>::const_iterator it;
        for (it = hm_table.begin(); it != hm_table.end(); it++) {
            pair<IntPairVector, int> hm_ent = *it;
            cout << "h[";
            print_tuple(hm_ent.first);
            cout << "] = " << hm_ent.second << endl;
        }
    }
    void print_tuple(IntPairVector &tup) const {
