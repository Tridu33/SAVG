...
void HMHeuristic::init_hm_table(IntPairVector &t) {
    map<IntPairVector, int>::iterator it;
    for (it = hm_table.begin(); it != hm_table.end(); it++) {
        pair<IntPairVector, int> hm_ent = *it;
        IntPairVector &tup = hm_ent.first;
    ...
    do {
        ...
        for (int op_id = 0; op_id < g_operators.size(); op_id++) {
            ...
            if (c1 != numeric_limits<int>::max()) {
                IntPairVector eff;
                vector<IntPairVector> partial_eff;
...
void HMHeuristic::extend_tuple(IntPairVector &t, const Operator &op) {
    map<IntPairVector, int>::const_iterator it;
    for (it = hm_table.begin(); it != hm_table.end(); it++) {
        pair<IntPairVector, int> hm_ent = *it;
        IntPairVector &entry = hm_ent.first;
...
int HMHeuristic::eval(IntPairVector &t) {
    vector<IntPairVector> partial;
...
int HMHeuristic::update_hm_entry(IntPairVector &t, int val) {
...
void HMHeuristic::generate_all_tuples(int var, int sz, IntPairVector &base) {
    if (sz == 1) {
        for (int i = var; i < g_variable_domain.size(); i++) {
            for (int j = 0; j < g_variable_domain[i]; j++) {
                IntPairVector tup(base);
...
void HMHeuristic::generate_all_partial_tuple(IntPairVector &base_tuple, IntPairVector &t,
                                             int index, int sz, vector<IntPairVector> &res) {
    if (sz == 1) {
...
int HMHeuristic::check_tuple_in_tuple(const IntPairVector &tup, const IntPairVector &big_tuple) {
