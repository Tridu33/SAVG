...
#   1. cps2FondandVerify/main.py     (generate FOND PDDL + capture Tg timing)
#   2. ./RunPRPForCurDomain.sh        (PRP solver + policy translation)
#   3. ./PrintHuman_Policy.sh         (re-translate policy to human format)
#   4. python ./DrawDotPolicy.py      (render policy graph)
#
# Collects generation timing (Tg), verification timing (Tv), solution size (|π|),
...
run_domain() {
    ...
    # --- 0. Generation step (always runs to capture Tg timing) ---
    if ! generate_domain "$domain"; then
        # Generation failed — check for existing files to fall back on
    ...
    # Safety check: autogenerated PDDL must exist after generation attempt
    ...
    # For Tg: use the captured value from TGS array (set by generate_domain)
    if [ -n "${TGS[$domain]:-}" ]; then
        tg_val="${TGS[$domain]}"
    else
        tg_val="—"
    fi
...
main() {
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            --generate)
                # Generation always runs (flag accepted for backward compatibility)
                shift
                ;;
            --help|-h)
                echo "Usage: $0"
                echo "  Generation (cps2FondandVerify/main.py) always runs to capture Tg."
                exit 0
                ;;
            *)
                shift
    ...
    echo "#  Generate: always (cps2FondandVerify/main.py)"
    echo "############################################################"
