(define (domain 3colorblocks)
    (:requirements :strips :equality :typing :derived-predicates)
    (:types BlockType)
    (:constants
        Bottom - BlockType
    )
    ...
    (:action colR
        ...
        :precondition (and (holding ?x) (isRed ?x))
        :effect (and (inRedBox ?x) (arm_empty) (not (holding ?x))))

    (:action colG
        :parameters (?x - BlockType)
        :precondition (and (holding ?x) (isGreen ?x))
        :effect (and (inGreenBox ?x) (arm_empty) (not (holding ?x))))

    (:action colB
        :parameters (?x - BlockType)
        :precondition (and (holding ?x) (isBlue ?x))
